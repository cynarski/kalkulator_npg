...
symbole = ['7', '8', '9', '/', '\u21BA', 'C', '4', '5', '6', '*', 'e','','1','2','3','-','^','','0','.','j','+','π']
...
def tworzeniePrzyciskow(okno, ekran,info):
    ...
    znak_cosinusa = tk.Button(okno, text='cos', borderwidth=0, bg='light gray',command= obliczCosinusa(pole_na_dane,ekran))
    znak_cosinusa.grid(row=len(ekran) + 7, column=1, ipady=5, ipadx=20)


    znak_tangensa = tk.Button(okno, text='tan', borderwidth=0, bg='light gray',command= obliczTangensa(pole_na_dane,ekran))
    znak_tangensa.grid(row=len(ekran) + 7, column=2, ipady=5, ipadx=20)

    znak_arcsin = tk.Button(okno, text='arcsin', borderwidth=0, bg='light gray',command=obliczArcsin(pole_na_dane,ekran) )
